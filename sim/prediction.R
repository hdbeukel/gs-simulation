...
# }
# 
# # visualize GP estimation by plotting actual values against estimated values
# gp.plot.estimation = function(trained.model, values){
#   if(class(trained.model) != "BGLR"){
#     stop("trained.model should be a BGLR object (call gp.train to train model)")
#   }
#   if(missing(values)){
#     values = trained.model$y
#   }
#   if(length(values) != length(trained.model$yHat)){
#     stop("number of values does not correspond to number of individuals in model")
#   }
#   # increase margin
#   old.mar = par()$mar
#   par(mar=c(5,6,3,3))
#   # plot responses obtained through estimated marker effects against actual values
#   est = trained.model$yHat
#   plot(x=values, y=est, xlab = "values", ylab = expression(estimates~(hat(y))))
#   # draw identity line
#   abline(0,1, col="red")
#   # add correlation
#   r = cor(values, est)
